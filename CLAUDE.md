...
- **Vercel correlation** — on Vercel, the `x-vercel-id` request header is published as `vercel-request-id` on spans, logs, metrics and error events by `src/lib/server/vercel-request-id.ts` (wired up in `sentry.server.config.ts` / `sentry.edge.config.ts`, plus `src/middleware.ts` for the edge runtime). Use it to line a Sentry event up with the matching Vercel invocation. No-op outside Vercel.

  It is set three ways because the SDK routes each signal differently: `Scope.setAttributes()` covers **logs and metrics** (both read the combined scope data); a `spanStart` client hook covers **spans** (scope attributes aren't applied to spans yet); and `Scope.setTag()` covers **error events** (`applyDataToEvent()` drops scope attributes, and `Event` has no attributes field — a tag is also what's searchable in Sentry). Any new signal added here should be checked against `@sentry/core`'s scope plumbing rather than assumed.
- **Shared trace ID with Vercel** — on Vercel, `src/lib/server/vercel-trace-context.ts` makes Sentry's request trace *continue* Vercel's, so one trace ID spans both tools (also wired up in `sentry.server.config.ts` / `sentry.edge.config.ts` / `src/middleware.ts`). No-op outside Vercel.

  Vercel does **not** hand the function a `traceparent` header or an OTLP endpoint. It publishes the invocation's context in-process at `globalThis[Symbol.for("@vercel/request-context")].get().telemetry.rootSpanContext` — an OTel `SpanContext` for Vercel's infra root span (same value `@vercel/otel` reads). Sentry's Node and Edge SDKs are OTel-based and parent the request span to whatever `propagation.extract()` returns, so we wrap the propagator Sentry registered and fall back to that span context. Two rules matter:

  - **An incoming trace wins.** If the request carries `sentry-trace`/`baggage` (every browser-SDK navigation does), we leave it alone — adopting Vercel's trace would sever pageload → server traces. `@vercel/otel` chooses the opposite (vercel/otel#107).
  - **Sampling stays ours.** Sentry inherits a remote parent's decision verbatim (`sampleSpan()` short-circuits on `parentSampled`), so we make the decision from the configured `tracesSampleRate` and pass it as the parent's trace flag; otherwise Vercel's flag would silently set the sample rate. When a `tracesSampler` is configured it still runs, so Vercel's flag is forwarded and the sampler decides.

  The request-context global is an undocumented runtime detail (the source calls it a placeholder for an unpublished `@vercel/request-context-storage` package) — validate what you read from it and never let it break tracing. Note the middleware span itself can't be re-parented: the edge SDK creates it before user code runs, so `src/middleware.ts` only sets the trace on the isolation scope for errors/logs raised there.
